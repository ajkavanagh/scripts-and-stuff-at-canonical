...
function find_floating_ip_for {
    ...
    floating_ips=$(openstack floating ip list -f json | jq -r '.[] | ."Floating IP Address"')
    addresses=$(openstack server show ${1} -f json | jq -r ".addresses.tinwood_admin_net | .[]")
	#_addr_line=$(openstack server show ${1} -f shell | grep addresses)
	if [[ "$?" == "1" ]]; then
		exit 1
	fi
    # now work through each line in the addresses, and if in the floating IP,
    # then return that one.
    for address in $addresses; do
        echo "trying $address"
        if [[ $floating_ips = *"$address"* ]]; then
            echo "Found it $address"
            floating_ip=$address
            return 0
        fi
    done
